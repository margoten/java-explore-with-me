...
public interface EndpointHitRepository extends JpaRepository<EndpointHit, Integer> {
    ...
    public List<ViewStats> getDistinctEndpointHits(LocalDateTime start, LocalDateTime end, List<String> uri);
    ...
    @Query(value = ""
            ...
            + "WHERE hit.uri IN ?3 AND hit.timestamp BETWEEN ?1 AND ?2 "
            + "GROUP BY hit.uri, hit.app ")
    public List<ViewStats> getEndpointHits(LocalDateTime start, LocalDateTime end, List<String> uri);
}
